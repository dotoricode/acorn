...
test('runInstall: 기존 lock 은 seed 가 건드리지 않음, acorn_version 은 현재 버전으로 스탬프 (§15 C1 비파괴)', () => {
  const w = makeWorkspace();
  try {
    const originalParsed = JSON.parse(readFileSync(w.lockPath, 'utf8')) as Record<string, unknown>;
    const git = makeFakeGit({ heads: new Map() });
    runInstall({
      lockPath: w.lockPath,
      harnessRoot: w.harnessRoot,
    ...
    const afterParsed = JSON.parse(readFileSync(w.lockPath, 'utf8')) as Record<string, unknown>;
    // tools / guard / schema_version 은 변경 없어야 함
    assert.deepEqual(afterParsed['tools'], originalParsed['tools'], 'tools 불변');
    assert.deepEqual(afterParsed['guard'], originalParsed['guard'], 'guard 불변');
    assert.equal(afterParsed['schema_version'], originalParsed['schema_version'], 'schema_version 불변');
    // acorn_version 은 현재 패키지 버전으로 갱신되어야 함
    const here = dirname(fileURLToPath(import.meta.url));
    const pkgVersion = (JSON.parse(readFileSync(join(here, '..', 'package.json'), 'utf8')) as { version: string }).version;
    assert.equal(afterParsed['acorn_version'], pkgVersion, 'acorn_version 이 현재 패키지 버전으로 스탬프되어야 함');
  } finally {
    w.cleanup();
  }
});
