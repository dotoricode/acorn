...
test('parseLock: schema_version 불일치 → LockError(SCHEMA)', () => {
  const bad = { ...VALID_LOCK, schema_version: 4 };
...
test('lockTemplatePath: 패키지 템플릿 파일 실존 (repo + 빌드 산출물 공통)', () => {
  ...
  // 템플릿은 v2 또는 v3 스키마 중 하나여야 함
  assert.ok(parsed.schema_version === 2 || parsed.schema_version === 3, 'template must be v2 or v3');
  if (parsed.schema_version === 2) {
    // v2: tools 구조 — placeholder 40-zero SHA
    for (const name of TOOL_NAMES) {
      assert.equal(parsed.tools[name].commit, '0'.repeat(40));
    }
  } else {
    // v3: providers 구조 — git-clone provider 는 placeholder 40-zero SHA
    const v3 = parsed as import('../src/core/lock.ts').HarnessLockV3;
    for (const [, entry] of Object.entries(v3.providers)) {
      if (entry.install_strategy === 'git-clone') {
        assert.equal(entry.commit, '0'.repeat(40));
      }
    }
  }
});
